	...
	{#if mode === 'choose'}
		...
		<div class="grid md:grid-cols-2 gap-6">
			...
			>
				...
				<h3 class="text-2xl font-bold text-black mb-2 group-hover:underline">Describe a Task</h3>
				<p class="text-black/60 font-medium mb-4">
					Tell the AI what you want to automate in plain English. The desktop agent will figure out the rest.
				</p>
			...
			<!-- Quick start option -->
			<button
				onclick={() => { mode = 'describe'; taskDescription = ''; name = ''; }}
				class="card-brutal p-8 hover:-translate-y-1 transition-transform text-left group cursor-pointer"
			>
				...
				<h3 class="text-2xl font-bold text-black mb-2 group-hover:underline">Quick Task</h3>
				<p class="text-black/60 font-medium">
					Give a short instruction and run it immediately. Great for one-off desktop tasks.
				</p>
			</button>
		</div>
	{:else}
